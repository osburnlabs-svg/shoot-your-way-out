...
| **`App.tsx` line 15 comment references `MapSelect` in routing flow** — comment reads "Loading → MainMenu → MapSelect → Game ⇄ Pause → GameOver"; MapSelect is removed from the screen flow. | Docs update 2026-05-12 | Comment only; no runtime impact | Phase 5 G1 — clean up on any incidental touch to `App.tsx` |
