...
| **Entity follow camera shipped (Phase 5 G1); tile/obstacle camera integration and zoom-level lock remain** — `CAMERA_ZOOM = 1.0` placeholder. Sprite scale constants still hardcoded per element. Full zoom can only be locked once tiles + enemies + HUD are visible together. | Phase 2 G3 (partial fix Phase 5 G1: 99bf87d→02acfad) | `CAMERA_ZOOM` constant in `gameConstants.ts`; inline per-entity screen position math | Phase 5 G2+ — tile rendering pass; lock zoom with full visual context |
