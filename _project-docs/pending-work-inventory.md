...
- **Gameplay speed-up after skill pickup** — ✅ Investigated and fixed 2026-05-23. Bug observed twice during Phase 7 testing: player/enemy/timer all accelerated dramatically after picking a skill at level-up, persisted for rest of run. First observation: Molotov skill (treated as one-off). Second observation: Energy Bar (passive stat), first skill of run, equipped pistol — confirmed bug is NOT skill-specific. Initial fix (commit 1bf4476) added sub-8ms frame guard targeting Reanimated vsync clustering theory — wrong theory, bug reproduced after fix. Second diagnostic traced actual root cause: `useFrameCallback` registration race. Inline arrow function passed to the hook created new function reference per React render; Reanimated's internal `useEffect([callback, autostart])` re-ran on each render, queueing cleanup + setup through async `runOnUI` bridge non-atomically; new callback registered before old one removed, leaving duplicate active IDs in `activeFrameCallbacks` for the rest of the run — RAF iterating both → tick fires 2× per vsync → everything 2× including timer. Fix shipped (commit 133ce6a): extracted callback into `useCallback(fn, [])` with explicit `'worklet'` directive, reverted the speculative sub-8ms guard back to `<= 0` (sub-8ms threshold was harmful at 120Hz). Trigger pattern: skill pickup produces `setState` burst via `handleSkillSelect` + `updateHudCounters`, causing rapid re-renders that activate the race. Bug was latent from inception of `useFrameCallback` usage — became observable only after Phase 7 HUD work expanded `setState` surface enough to trigger the race detectably.

  Session 3 (2026-05-25): Mo asked CC to investigate entire codebase for a fix. CC shipped `if (dtMs < 8) return` guard (commit 9f47bd2) with new theory — pan gesture triggering `__flushAnimationFrame` between vsyncs, causing extra mid-vsync invocations with small dtMs to inflate `tickAccMs`. Mo device-tested across 10 runs: 8 clean, 2 reproduced bug. Fix did not address mechanism. Reverted via hard reset back to ae61bd6. **This is the second failed attempt at the sub-8ms guard fix.** First attempt was commit 1bf4476 (session 1). Both attempts failed. The sub-8ms guard is also actively harmful for 120Hz users — real 120Hz frame intervals are ~8.33ms, uncomfortably close to the 8ms cutoff. Do not re-attempt this fix without different evidence. Updated current best hypothesis after session 3: still multi-registration accumulation per session 2 framing. Sub-8ms guard does not address the mechanism — whatever is adding registrations is unaffected by frame-dt filtering at the consumption side. State of code: ae61bd6.
