...
4. **Pond as static prop.** ⏸️ Deferred during Phase 6 planning (session 2026-05-20). Sprite source confirmed (`_project-docs/kits/tds-modern-pixel-game-kit/tds-modern-tilesets-environment/PNG/Tiles/_0000_WTiles.png`, 320×320 native). Original inventory plan (new WATER_POOL + buildWaterFeatures function + 2-per-run deterministic placement) conflicted with constraints Mo articulated during planning: (a) do not raise the asset budget — the 70–100 vegetation count and equivalent budgets on other pools are load-bearing for performance after the Phase 5 stutter work; (b) do not reduce existing prop category spawn counts — rocks, vegetation, and wrecks all look correct at current density; (c) do not add new custom spawn logic (helicopter and plane wrecks are existing one-offs; do not add a third); (d) barrel-displacement is not viable because barrels have proximity-to-building placement logic. Investigated paths: adding to VEGETATION_POOL violates (b); new pool violates (a); clustering with probability gate violates (c). 320×320 collision footprint also significantly larger than any existing collision shape (~2× large trees). Disposition decision deferred to a fresh session. Decision to be made: implement with one constraint relaxed (which), defer to v1.1, or scrap.
